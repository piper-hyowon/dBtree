...
func (s *smtpService) SendOTP(ctx context.Context, to string, otpCode string) error {
	if ctx.Err() != nil {
		return ctx.Err()
	}
	...
	htmlBody := fmt.Sprintf(emailTemplateOTP, otpCode, common.OTPExpirationMinutes)

	return s.sendEmail(ctx, to, subject, htmlBody)
}

func (s *smtpService) SendWelcome(ctx context.Context, to string) error {
	if ctx.Err() != nil {
		return ctx.Err()
	}
...
func (s *smtpService) sendEmail(_ context.Context, to, subject, htmlBody string) error {
